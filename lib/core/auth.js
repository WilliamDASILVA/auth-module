...
export default class Auth {
  ...
  reset () {
    if (!this.strategy.reset) {
      ...
      this.setClient(this.$state.strategy, false)
      this.setUid(this.$state.strategy, false)
  ...
  getClient (strategy) {
    const _key = '_client.' + strategy

    return this.$storage.getUniversal(_key)
  }

  getUid (strategy) {
    const _key = '_uid.' + strategy
  ...
  setClient (strategy, client) {
    const _key = '_client.' + strategy

    return this.$storage.setUniversal(_key, client)
  }
  ...
  request (endpoint, defaults) {
    ...
    console.log('do request', _endpoint)
  ...
  requestWith (strategy, endpoint, defaults) {
    ...
    const client = this.getClient(strategy)
    const uid = this.getUid(strategy)
    ...
    if (!_endpoint.headers['Access-Token'] && isSet(token) && token) {
      _endpoint.headers['Access-Token'] = token
      console.log('client', client)
      console.log('uid', uid)
      if (client) _endpoint.headers['Client'] = client
      if (uid) _endpoint.headers['Uid'] = uid
      console.log('endpoints', _endpoint)
    }
